...
1. [ ] Project Schema Standardization — Normalize ProjectData field naming for consistency and extensibility. Renames: projectDate → dateProject (matches dateCreated/dateUpdated YYYYMMDD convention), slice/task file fields → fileSlice/fileTasks (verb-consistent with each other). Add optional higher-level reference fields: fileHLD, fileArch, fileOverview — at least one of these is useful for most work since context assembly currently only targets slice+tasks, but architectural and design work frequently needs broader project context. The context assembly engine should be aware of these fields and include the referenced documents when they're populated. Requires: schema migration for existing projects, MCP tool input schema updates (project_update), template engine updates to consume new fields. Dependencies: [MCP Server — State Update Tools]. Effort: 3/5

2. [ ] Command Grammar and Slash Commands — Define a structured command grammar for Context Forge operations that parses identically across interaction surfaces (terminal CLI, Claude Code slash commands, MCP tool calls). Examples: cf build --slice auth --instruction design, cf set slice auth, cf set phase implementation, cf task-breakdown. The grammar provides consistency and precision — a compact, unambiguous alternative to natural-language descriptions of desired operations. Implementation options: a packages/cli addition to the monorepo, compound MCP tools that accept command strings, or both backed by a shared parser. The grammar definition itself (command vocabulary, flags, argument types) should be designed before implementation. Dependencies: [MCP Server — State Update Tools, Project Schema Standardization (for field names)]. Effort: 4/5

3. [ ] MCP Server — Streamable HTTP Transport — Add a Streamable HTTP endpoint to the MCP server alongside the existing stdio transport. Same tool registrations, second transport binding. Enables multi-client access (Electron + Claude Code against one server instance), long-lived agent sessions that survive individual agent spawns, and remote/team server deployments. Configuration: port selection, optional auth token. The hybrid launch model from the architecture doc (Electron connects to already-running server) becomes fully functional with this slice. Dependencies: [MCP Server — State Update Tools]. Risk: Low (SDK supports both transports with same API surface). Effort: 2/5
