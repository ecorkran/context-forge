...
12. [x] **(192) Default Worktree Aggregation & Field Reset** — Two improvements that complete the worktree UX. (1) Default worktree aggregation: when listing from the default worktree, scan all registered worktree paths and union the results so the default worktree serves as a true project-wide overview even when branches haven't merged. Currently the default worktree only sees files on its own filesystem. (2) Field reset (`cf unset`): a new command to clear fields back to unset state. Works on both project-level and worktree-scoped fields. Use case: returning a project to first-run state, clearing stale artifact references. `cf unset <field>` is preferred over `cf set <field> ""` to avoid ambiguity — `cf set<enter>` and `cf set <space><enter>` must not accidentally unset anything.

   **Value:** Default worktree aggregation delivers the "dashboard" promise — seeing all initiatives across all worktrees from the project root. Field reset eliminates the clunky workaround of setting fields to spaces and makes project state management explicit.
