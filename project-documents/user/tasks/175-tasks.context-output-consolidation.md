...
- [ ] Add optional fields to `ContextData` in `packages/core/src/types/context.ts`:
  - `fileArch?: string`
  - `fileSlicePlan?: string`
  - `fileHLD?: string`
  - `fileSpec?: string`
- [ ] Verify `EnhancedContextData` inherits them via `extends` (no additional changes needed)
- [ ] Fix any TypeScript compilation errors caused by the new fields (check all files that construct `ContextData` objects — test helpers, factories, etc.)
...
- [ ] In `packages/core/src/services/ContextIntegrator.ts`, update `mapProjectToEnhancedContext()`:
...
- [ ] Update `mapProjectToContext()` (legacy mapper) with same fields
- [ ] Update tests in `packages/core/tests/services/ContextIntegrator.test.ts`:
...
- [ ] In `packages/core/src/services/TemplateProcessor.ts`, update `createEnhancedData()`:
...
- [ ] Update tests in `packages/core/tests/services/TemplateProcessor.test.ts`:
...
- [ ] In `packages/core/src/services/SectionBuilder.ts`, rewrite `buildProjectInfoSection()`:
...
- [ ] Update tests in `packages/core/tests/services/SectionBuilder.test.ts`:
...
- [ ] In `packages/core/src/services/constants.ts`:
...
- [ ] In `packages/core/src/services/ContextTemplateEngine.ts`, update `buildTemplate()`:
...
- [ ] Update tests in `packages/core/tests/services/ContextTemplateEngine.test.ts`:
...
- [ ] Verify the spec is already captured in the slice design (Change 7 section) — if so, mark this task as confirming it is complete and accurate
...
- [ ] Check off slice 175 in `160-slices.project-workflow-system.md`
