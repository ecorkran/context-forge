...
- [x] **1.1 Create `ResolvedProject` type and `resolveProject()` function**
  ...
  - [x] Throws `Error` when worktreeId provided but worktree not found
  - [x] Returns `null` when project not found
  - [x] Returns unmodified project when worktreeId omitted
  - [x] `resolvedWorktree` field present only when overlay applied

- [x] **1.2 Export `resolveProject` and `ResolvedProject` from core**
  - File: `packages/core/src/index.ts`
  - Add export line: `export { resolveProject, type ResolvedProject } from './services/projectResolver.js';`
  - [x] `resolveProject` importable from `@context-forge/core`
  - [x] `ResolvedProject` type importable from `@context-forge/core`
  - [x] TypeScript compiles (`npx tsc --noEmit` from `packages/core`)

- [x] **1.3 Unit tests for `resolveProject()`**
  ...
  - [x] `npm test` from `packages/core` passes (no regressions)

- [x] **1.4 Commit: core resolveProject function and tests**
...
- [x] **2.1 Update `workflow_status` to use `resolveProject()`**
  ...
  - [x] `workflow_status` without `worktreeId` behaves identically to before
  - [x] `workflow_status` with valid `worktreeId` returns same overlay-applied results as before
  - [x] `workflow_status` with invalid `worktreeId` returns clear error message

- [x] **2.2 Update `workflow_next` to use `resolveProject()`**
  - File: `packages/mcp-server/src/tools/workflowTools.ts`
  ...
  - [x] `workflow_next` without `worktreeId` behaves identically
  - [x] `workflow_next` with valid `worktreeId` returns same results
  - [x] `workflow_next` with invalid `worktreeId` returns clear error

- [x] **2.3 Clean up imports in workflowTools.ts**
...
- [x] **2.4 Tests for workflow tools migration**
  ...
  - [x] Worktree resolution verified for both tools

- [x] **2.5 Commit: workflow tools migration**
  - Commit from project root
...
- [x] **3.1 Add `worktreeId` parameter to `project_get` input schema**
  ...
  - [x] `project_get` without `worktreeId` returns identical response to before
  - [x] `project_get` with valid `worktreeId` returns overlay-applied top-level fields
  - [x] `project_get` with valid `worktreeId` includes `resolvedWorktree: { id, name }` in response
  - [x] `project_get` with invalid `worktreeId` returns clear error
  - [x] Introspection reflects worktree's active slice when `worktreeId` provided

- [x] **3.2 Tests for project_get worktree resolution**
...
- [x] **3.3 Commit: project_get worktree resolution**
  - Commit from project root
...
- [x] **4.1 Update `context_build` to use `resolveProject()`**
  ...
  - [x] `context_build` without worktree produces identical output
  - [x] `context_build` with worktree produces identical output (diff test)
  - [x] No inline worktree lookup/overlay logic remains in the handler

- [x] **4.2 Update `context_summarize` to use `resolveProject()`**
  - File: `packages/mcp-server/src/tools/contextTools.ts`
  ...
  - [x] `context_summarize` without worktree produces identical output
  - [x] `context_summarize` with worktree produces identical output

- [x] **4.3 Clean up imports in contextTools.ts**
...
- [x] **4.4 Tests for context tools migration**
  - Run existing MCP tests: `npm test` from `packages/mcp-server`
  - Verify context output parity: generate context with worktree before and after changes, compare outputs
  - [x] All existing MCP tests pass
  - [x] Context output identical for worktree and non-worktree cases

- [x] **4.5 Commit: context tools migration**
  - Commit from project root
...
- [x] **5.1 Update `cf next` (next.ts) to use `resolveProject()`**
  ...
  - [x] `cf next` without worktree behaves identically
  - [x] `cf next` with worktree shows resolved overlay values

- [x] **5.2 Update `cf build` (build.ts) to use `resolveProject()`**
  ...
  - [x] `cf build` without worktree produces identical output
  - [x] `cf build` with worktree produces identical output

- [x] **5.3 Update `cf status` (status.ts) to use `resolveProject()`**
  ...
  - [x] `cf status` without worktree shows same summary as before
  - [x] `cf status` with worktree shows resolved values

- [x] **5.4 Update `cf arch list` (arch.ts) to use `resolveProject()`**
  ...
  - [x] `cf arch list` with worktree behaves identically

- [x] **5.5 Update `cf plan list` (plan.ts) to use `resolveProject()`**
  ...
  - [x] `cf plan list` with worktree behaves identically

- [x] **5.6 Update `cf slice list` (slice.ts) to use `resolveProject()`**
  - File: `packages/cli/src/commands/slice.ts`
  - Replace `applyWorktreeOverlay(rawProject, worktreeId)` call (line 32) with `resolveProject()`
  - [x] `cf slice list` with worktree behaves identically

- [x] **5.7 Update `cf tasks` (task.ts) to use `resolveProject()`**
  - File: `packages/cli/src/commands/task.ts`
  - Two call sites (lines 38 and 74) — replace both with `resolveProject()`
  - [x] `cf tasks list` with worktree behaves identically
  - [x] `cf tasks items` with worktree behaves identically

- [x] **5.8 Update `cf project set` display (project.ts) to use `resolveProject()`**
  ...
  - [x] `cf set` with worktree resolves file indices correctly

- [x] **5.9 Tests for CLI commands migration**
  - Run existing CLI tests: `npm test` from `packages/cli`
  - [x] All existing CLI tests pass
  - [x] No regressions in worktree-related CLI behavior

- [x] **5.10 Commit: CLI commands migration**
  - Commit from project root
...
- [x] **6.1 Remove unused `applyWorktreeOverlay` imports from consumer files**
  ...
  - [x] No unused `applyWorktreeOverlay` imports in modified files
  - [x] TypeScript compiles cleanly

- [x] **6.2 Verify no inline overlay calls remain in migrated code**
