...
- [ ] **6. Test: `getNext()` #56 regression, wording, gate-ordering, and TD-2a propagation** — Effort: 2/5
  ...
  - [ ] Add a wording test: an active slice with derived status `in-progress` (partial task completion) produces a recommendation containing "continue"/"Continue," not "advance to"; an active slice with derived status `complete` produces "advance to." Assert on the actual returned string content, not just the selected entry.
