...
- [x] 1. Move the three range helpers from the CLI into core
  - [x] Move `isInIndexRange()`, `getWorktreeIndexRange()`, and
  ...
  - [x] Success: `pnpm --filter @context-forge/core build` succeeds and the
        three symbols are importable from `@context-forge/core`.

- [x] 2. Convert the CLI module to a re-export shim
  - [x] In `packages/cli/src/utils/worktree-overlay.ts`, replace the three
  ...
  - [x] Do NOT edit any of the nine importing command files
  ...
  - [x] Success: `pnpm -r build` clean, and
  ...
  - [x] If any behavioral diff appears, stop. A pure move that changes behavior
        means there is a hidden coupling that must be understood before
        proceeding — do not work around it.

- [x] 3. Commit the helper relocation
  - [x] Commit message: `refactor(core): move worktree index-range helpers into core`
  - [x] Success: working tree clean, build green, full cli + core suites pass.

- [x] 4. Relocate the `ResolvedProject` interface to `types/project.ts`
  - [x] Move the `ResolvedProject` interface declaration (currently
  ...
  - [x] Add `ResolvedProject` to the type export on
        `packages/core/src/types/index.ts` line 16, alongside `ProjectData`.
  - [x] In `projectResolver.ts`, import the type and re-export it so
  ...
  - [x] Do NOT add `resolvedWorktree` to `ProjectData` itself. It is a
        resolution artifact, not persisted state; adding it would leak into the
        storage schema.
  - [x] Note: `packages/cli/src/utils/project.ts` declares its own unrelated
        local `ResolvedProject` (`{ id, source }`). Different module, no
        collision. Do not merge or rename the two.
  - [x] Success: `pnpm -r build` clean with zero edits to any consumer of
        `ResolvedProject`.

- [x] 5. Widen `getNext()`'s parameter type
  - [x] In `packages/core/src/introspection/WorkflowNavigator.ts`, change
        `async getNext(project: ProjectData)` (line ~132) to accept
        `ResolvedProject`, importing the type from `../types/index.js`.
  - [x] Because `resolvedWorktree` is optional, plain `ProjectData` remains
        assignable — no call site changes, and no existing test that passes a
        `makeProject()` result needs editing.
  - [x] Success: `pnpm -r build` clean and `pnpm -r test` green for core, cli,
...
- [x] 6. Commit the type relocation
  - [x] Commit message: `refactor(core): move ResolvedProject to types and widen getNext signature`
  - [x] Success: working tree clean, build green, all three suites pass.

### Part 2 — The band-warning fix

- [x] 7. Replace the hundred-block band check with tiered resolution
  - [x] In `WorkflowNavigator.ts`, extract the band-warning logic out of
  ...
  - [x] Implement the three tiers exactly as specified in LLD Decision 1:
    ...
    4. No worktrees configured → existing `hundredBlock(index)` vs
       ...
       today's.
  - [x] Use the core `isInIndexRange()` helper for every containment test. Do
        not re-implement the comparison inline — that is the duplication Task 1
        exists to prevent.
  - [x] Emit at most one band warning in every tier. The tiers are mutually
        exclusive by construction; do not let two fire.
  - [x] Message formats (LLD Decision 2), using an ASCII hyphen for ranges to
  ...
  - [x] `hundredBlock()` must survive, used only by the tier-4 branch.
  - [x] Success: `pnpm -r build` clean. The existing band-warning tests at
...
- [x] 8. Add unit tests for the tiered band check
  - [x] Add to the existing `describe('arch-existence and index band warnings', …)`
  ...
  - [x] Case 1 (#48 regression): worktree `default [100,799]` active, slice 209,
        arch `100-arch.*` → assert `next.warnings` is undefined. Reference #48
        in a comment so the regression's origin is traceable.
  - [x] Case 2: worktrees `default [100,199]` and `api [200,299]`, `default`
  ...
  - [x] Case 3: worktree `default [100,199]` with `rangeOverride: true` active,
        slice 209 → assert no warning.
  - [x] Case 4: worktree `default [100,799]` configured, no `resolvedWorktree`,
        slice 209 → assert no warning.
  - [x] Case 5: worktree `default [100,799]` configured, no `resolvedWorktree`,
        slice 850 → assert exactly one warning; assert it contains the range
        listing and contains neither `band` nor `hundred`.
  - [x] Case 6: no worktrees, slice 209, arch `100-arch.*` → assert the legacy
        ...
        duplicate it.)
  - [x] Do NOT split `WorkflowNavigator.test.ts` as part of this slice. It is
        already 1400 lines and the split is tracked as its own maintenance item.
  - [x] Success: all six cases pass; full core suite green (expect ~1032 + new
        cases).

- [x] 9. Commit the fix
  - [x] Commit message: `fix(core): honor worktree indexRange in cf next band warning (#48)`
  - [x] Success: working tree clean, build green, all three suites pass.

### Part 3 — Verification and close-out

- [x] 10. Manual end-to-end verification walkthrough
  - [x] Run `pnpm -r build`, then execute the Verification Walkthrough section
        of the LLD against a scratch project using the freshly built local CLI
        (`node packages/cli/dist/index.js`).
  - [x] Confirm each step's stated expectation, specifically:
  ...
  - [x] Update the LLD's Verification Walkthrough section **in place** with the
        real commands and actual output observed, replacing the draft. This is
        the Phase 6 refinement the LLD calls for.
  - [x] Success: every step confirmed against real output, walkthrough updated,
...
- [x] 11. Full verification pass
  ...
  - [x] Run `cf check` (or `workflow_check`) scoped to slice 919 and confirm
        zero findings for this slice.
  - [x] Success: all of the above verified, with actual command output read
        rather than assumed.

- [x] 12. Close out GitHub issues #48 and PR #49
  - [x] Close #48, referencing the fix commit.
  - [x] Close PR #49 (`jakez-gh:fix/band-warning-honors-worktree-range`) with an
  ...
  - [x] Do not close #48's "adjacent suggestion" about dotted sub-index
        numbering — that is a separate discussion the reporter said they would
        file independently.
  - [x] Success: both closed with the acknowledgement posted; nothing else in
        the reporter's thread silently dropped.

- [x] 13. Documentation and status updates
  - [x] Add a `CHANGELOG.md` entry under `[Unreleased]` describing the #48 fix
        in user-facing terms.
  - [x] Set this task file's frontmatter `status` to `complete`.
  - [x] Set the LLD's frontmatter `status` to `complete`.
  - [x] Check off entry 19 `(919)` in
        `user/architecture/900-slices.maintenance-and-refactoring.md`.
  - [x] Success: `cf list slices` renders 919 as `✓ complete` (not
        `● tasks done` — that display distinguishes a ticked plan checkbox from
        merely-finished tasks).

- [x] 14. Final commit
  - [x] Commit message: `docs: complete slice 919 (band warning respects worktree indexRange)`
