...
- [ ] 2.1 Add `GateEvaluation` and `evaluateReviewGate()` to `reviewGate.ts`
...
- [ ] 2.2 Refactor `WorkflowNavigator.evaluateGate()` to delegate
...
- [ ] 2.3 Test: regression — 241's full navigator suite passes unchanged
...
- [ ] 2.4 Test: `evaluateReviewGate()` directly
...
- [ ] 2.5 Commit checkpoint — extracted evaluator
...
- [ ] 3.1 Add optional `ConfigManager` constructor parameter
...
- [ ] 3.2 Test: constructor accepts optional config with no behavior change
...
- [ ] 3.3 Commit checkpoint — optional config dependency
...
- [ ] 4.1 Hoist gate config resolution in `check()` and `checkAll()`
...
- [ ] 4.2 Implement `ruleReviewGate` (the new rule)
...
- [ ] 4.3 Test: absent review → warning
...
- [ ] 4.4 Test: failing verdict → error
...
- [ ] 4.7 Test: gating off → identical to pre-242 output
...
- [ ] 4.8 Test: present-but-no-verdict (UNKNOWN) under default policy
...
- [ ] 4.9 Test: `cf check --fix` does not touch review-gate findings
...
- [ ] 4.10 Commit checkpoint — review-gate rule
...
- [ ] 5.1 Wire `ConfigManager` into `cf check`
...
- [ ] 5.2 Wire `ConfigManager` into `workflow_check` (MCP)
...
- [ ] 5.3 Test: end-to-end — both surfaces report the same finding
...
- [ ] 5.4 Commit checkpoint — surface wiring
...
- [ ] 6.1 Build and run the full suite
...
- [ ] 6.2 Execute the design Verification Walkthrough
...
- [ ] 6.3 Update docs/frontmatter and commit
