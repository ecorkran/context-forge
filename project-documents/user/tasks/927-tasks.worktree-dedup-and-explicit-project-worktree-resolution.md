...
- [x] **Task 6: Export `stripTrailingSeparator`** (effort 1)
  - [x] In `packages/core/src/utils/worktree-overlay.ts`, export the existing private `stripTrailingSeparator` (do not duplicate it)
  - [x] Success: core build passes

- [x] **Task 7: Add `replaceRoot` and `dedupKey` to the merge** (effort 2)
  - [x] In `packages/core/src/introspection/mergeCheckResults.ts`, add a module-private `replaceRoot(text, viewRoot)`. It replaces each occurrence of `stripTrailingSeparator(viewRoot)` with a fixed token, but only where a path separator or end of string follows (so `/repo` does not match inside `/repo-other`)
  - [x] Add a module-private `dedupKey(finding, viewRoot)` as shown in the design (`#100 — Dedup Key Normalization`): `rule | norm(location) | norm(description)`
  - [x] In the merge loop, use `dedupKey(finding, result.projectPath)` in place of the inline key
  - [x] The kept finding is pushed unmodified. Normalization affects the key only.
  - [x] Keep the `results.length === 1` early return exactly as is
  - [x] Update the function's JSDoc: dedup key is root-normalized per view
  - [x] Success: core build passes

- [x] **Task 8: Unit-test the normalized merge** (effort 2)
  - [x] Create `packages/core/tests/introspection/mergeCheckResults.test.ts` (no core test file exists for the merge today; CLI's `check-worktree-attribution.test.ts` covers attribution and stays as is)
  - [x] Cases:
...
- [x] **Task 9: Commit Part 2**
  - [x] `pnpm -r build` and `pnpm -r test` pass (CLI attribution tests must still pass unchanged)
  - [x] Commit: `fix(core): normalize checkout root in check-result dedup key` with `Fixes #100` in the body

## Part 3 — #100 Fix Path: Per-View Fixes and `mergeFixResults`

- [x] **Task 10: Make attribution generic over the result type** (effort 1)
  - [x] In core `mergeCheckResults.ts`, make `attributeFindings` generic: `<T extends ConsistencyCheckResult>(result: T, …): T`
  - [x] In `packages/cli/src/commands/check.ts`, make `runAttributed` generic the same way
  - [x] Success: `pnpm -r build` passes; no behavior change

- [x] **Task 11: Add `mergeFixResults` to core** (effort 2)
  - [x] In `mergeCheckResults.ts`, add exported `mergeFixResults(results: ConsistencyFixResult[], invokingPath?: string): ConsistencyFixResult`
  - [x] Finding dedup delegates to `mergeCheckResults` (no second copy of the loop)
  - [x] `fixed` is summed; `fixLog` and `fixErrors` are concatenated in result order, with no dedup
  - [x] Export it from `packages/core/src/introspection/index.ts` next to `mergeCheckResults`
  - [x] Success: core build passes

- [x] **Task 12: Unit-test `mergeFixResults`** (effort 1)
  - [x] In `mergeCheckResults.test.ts`, add cases: `fixed` summed across two results; `fixLog` entries from both views present (two entries for the same logical fix in two checkouts); `fixErrors` concatenated; findings deduped identically to `mergeCheckResults`; single result passes through with fix fields intact
  - [x] Success: core tests pass

- [x] **Task 12a: Commit checkpoint**
  - [x] `pnpm -r build` and `pnpm --filter @context-forge/core test` pass
  - [x] Commit: `feat(core): add mergeFixResults for per-view fix merging`

- [x] **Task 13: CLI single-slice fix mode applies per view** (effort 2)
  - [x] In `check.ts` single-slice path: when `fixMode`, run `checker.fix(v)` per view via `runAttributed`, then `mergeFixResults(…, invokingPath)`; otherwise unchanged (`check` + `mergeCheckResults`)
  - [x] Remove the post-merge `checker.applyFixes(merged)` call
  - [x] Success: CLI build passes

- [x] **Task 14: CLI all-slices fix mode applies per view** (effort 3)
  - [x] Keep the dry-run results **per view** (`dryRunResults`) as well as the merged `dryRun` used for display and the confirmation count
  - [x] Zero-fixable and abort paths are unchanged
  - [x] On confirm (or `--yes`): run `checker.applyFixes(r)` on each per-view dry-run result (no re-check), then `mergeFixResults(…, invokingPath)`
  - [x] Remove both post-merge `checker.applyFixes(dryRun)` calls
  - [x] Success: CLI build passes

- [x] **Task 15: Test CLI per-view fixing** (effort 3)
  - [x] `check.test.ts` and `check-worktree-attribution.test.ts` both mock `ConsistencyChecker` entirely — this test needs the real implementation running against real files, so it does not fit either as-is. Follow 926 Task 5's precedent: use the real `ConsistencyChecker` over real temp directories (mock only the project store), not a mocked checker
  - [x] Build a project with two real worktree checkouts, each containing a `project-documents/.../900-slices.*.md` fixture with the same unchecked entry for a complete slice (same fixable state in both checkouts)
  - [x] Assert for single-slice `--fix`: both checkout files are fixed on disk; `fixLog` has two entries with different file paths
  - [x] Assert for all-slices `--fix --yes`: same; the displayed finding appears once
  - [x] This test must fail against pre-Task-13/14 code (confirm by running it against the current tree before Task 13's change, or by inspecting that the old post-merge `applyFixes` path would fix only one checkout)
  - [x] Success: CLI tests pass

- [x] **Task 15a: Commit checkpoint**
  - [x] `pnpm -r build` and `pnpm --filter @context-forge/cli test` pass
  - [x] Commit: `fix(cli): apply check fixes per worktree before merging`

- [x] **Task 16: MCP `workflow_check` uses `mergeFixResults`** (effort 1)
  - [x] In `packages/mcp-server/src/tools/workflowTools.ts`, in both fix branches (single-slice and all-slices), use `mergeFixResults` when `fixMode`, `mergeCheckResults` otherwise
  - [x] Success: MCP build passes

- [x] **Task 17: Test MCP fix fields survive the merge** (effort 1)
  - [x] In `packages/mcp-server/tests/workflowTools.test.ts`, add a two-worktree `workflow_check` case with `fix: true` asserting the JSON result contains `fixed`, `fixLog`, and `fixErrors` with entries from both views
  - [x] Success: MCP tests pass

- [x] **Task 18: Commit Part 3**
  - [x] `pnpm -r build` and `pnpm -r test` pass
  - [x] Commit: `fix(mcp): merge per-view fix results in workflow_check`

## Part 4 — Verification and Close-Out

- [x] **Task 19: Verification walkthrough** (effort 2)
  - [x] Use the local build (`node packages/cli/dist/index.js`), not the global `cf`
  - [x] Run the walkthrough steps from the slice design (`Verification Walkthrough`, steps 1–7) against this repo with a temporary worktree at `/tmp/cf-wt-927`
  - [x] Replace the design's walkthrough outline with the actual commands and their real output
  - [x] Remove the temporary worktree and unregister it (`cf worktree rm`), and confirm `git worktree list` no longer shows it
  - [x] Success: every walkthrough step shows the expected result; any mismatch is stopped and reported, not worked around

- [x] **Task 20: Final validation and commit** (effort 1)
  - [x] `pnpm -r build`, `pnpm -r test`, and lint (if configured) pass
  - [x] Run `node packages/cli/dist/index.js check` on this repo and confirm no new findings from this slice's docs
  - [x] Commit the walkthrough update: `docs: add slice 927 verification walkthrough results`
  - [x] Hand off for merge and the design's success-criteria check-off. Do not merge, tag, or close issues without PM direction.
