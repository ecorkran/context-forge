...
- [x] Create `packages/mcp-server/src/tools/stateTools.ts`
- [x] Export a `registerStateTools(server: McpServer)` function (same pattern as `registerProjectTools` and `registerContextTools`)
- [x] Add imports:
...
- [x] Register `context_summarize` tool in `registerStateTools()`:
...
- [x] Wrap handler in try/catch — surface store failures via `errorResult`
- [x] File compiles (`pnpm --filter context-forge-mcp build`)
...
- [x] Create `packages/mcp-server/src/__tests__/stateTools.test.ts`
- [x] Mock `@context-forge/core/node` to stub `FileProjectStore` (same pattern as `projectTools.test.ts`)
- [x] Mock `getById` and `update` methods on the store
- [x] Create mock `ProjectData` fixture with existing `customData` fields (`recentEvents`, `additionalNotes`, `monorepoNote`, `availableTools`)
- [x] Set up test client using `InMemoryTransport` + `Client` pattern (import from `@modelcontextprotocol/sdk`)
- [x] Test cases:
  - [x] Returns updated project as JSON after updating `recentEvents`
  - [x] Preserves other `customData` fields (`monorepoNote`, `availableTools`) during update
  - [x] Updates `additionalNotes` when provided
  - [x] Does not modify `additionalNotes` when not provided in the call
  - [x] Returns `isError: true` for non-existent project ID (message references `project_list`)
  - [x] Returns `isError: true` for empty/whitespace-only summary
  - [x] Returns `isError: true` on store update failure (mock `update` throws)
- [x] Run `pnpm --filter context-forge-mcp test` — all tests pass (new + existing)
...
- [x] Run `pnpm --filter context-forge-mcp test` — all MCP server tests pass (project tools, context tools, state tools, lifecycle)
- [x] Verify no regressions: existing tests pass unchanged
...
- [x] Commit all changes: "feat(mcp-server): implement context_summarize tool with tests (slice 147)"
- [x] Update DEVLOG.md with implementation summary and commit hashes
- [x] Mark slice 147 (item 8) as `[x]` in `140-slices.context-forge-restructure.md`
- [x] Update slice design status from `not started` to `complete`
