...
## Task 4 — WorkflowNavigator: config injection and gate evaluation (design TD-3, TD-5)

- [ ] 4.1 Add optional `ConfigManager` constructor dependency
  ...
  - No behavior change yet — the field is unused until 4.2.
  - Success: compiles; all existing no-arg `new WorkflowNavigator()` call sites still valid.

- [ ] 4.2 Evaluate the gate inside `deriveSliceStatus()` (code + slice + tasks boundaries)
  - Introduce a private helper (e.g. `evaluateGate(project, projectPath, boundary): Promise<GateStatus | null>`) that: calls `resolveGateConfig(this.config)`; if `null` (gating off or no config) returns `null` (caller keeps existing status). Otherwise derives the reviewType via `positionToReviewType(boundary)`, calls `detectDocuments(projectPath, index, reviewType)`, and:
    ...
    - else `parseFrontmatter(join(projectPath, review))` → `evaluateVerdict(normalizeVerdict(data.verdict), threshold, unknownAs)`; `clears` → `null` (keep going/complete), `failed` → return `review-failed`.
...
- [ ] 4.3 Evaluate the pre-slice-plan (`arch`) boundary in the no-active-slice path
...
- [ ] 4.4 Fill the reserved `LIFECYCLE: review-gate` branch in `getNext()` (TD-7)
...
- [ ] 4.5 Test: navigator gate behavior (all boundaries)
  - Construct the navigator with a stub `ConfigManager` and point `project.projectPath` at a fixture (Task 6). For each boundary assert: absent artifact → `pending-review`/`review`; `FAIL` → `review-failed`/`blocked`; clearing verdict → prior status/recommendation unchanged; correct reviewType is sought per boundary.
  - Assert `review-failed` rationale contains the verdict and threshold; `pending-review` rationale names the review type.
  - Success: each boundary has clears/pending/failed coverage.

- [ ] 4.6 Test: gating-off regression (conservative-by-default)
...
- [ ] 5.1 Wire `ConfigManager` into CLI `next` and `status`
...
- [ ] 5.2 Wire `ConfigManager` into MCP workflow tools
...
- [ ] 5.3 Test: surfaces pass config; existing CLI/MCP tests pass
...
## Task 6 — Test fixtures (verdict-bearing reviews)

- [ ] 6.1 Add verdict-bearing review fixtures
  - The slice 240 fixtures under `core/tests/fixtures/introspection/project/.../reviews/` carry NO `verdict` field (discovery-only). Add fixtures with `verdict` frontmatter for the boundaries under test. At minimum, for the `code` type: a `PASS`, a `CONCERNS`, a `FAIL`, and a no-`verdict` artifact; plus an absent-file case (simply do not create the file for that index). Add at least one `arch` and one `slice` verdict fixture to exercise those boundaries.
  ...
  - Success: fixtures exist; no unrelated fixture-count test breaks (fix counts if the shared tree grew).
...
- [ ] 7.3 Update docs/frontmatter and commit
  ...
  - Commit on the slice branch with a semantic message. Do NOT run the code review here — that is a separate step.
  - Success: docs updated; work committed on the slice branch.
