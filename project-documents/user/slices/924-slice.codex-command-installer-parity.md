...
- **D5 — Scope flags.** ~~Project-local by default, `--global` to opt out — matching `cf config set --global` polarity. No `--local` flag; `--project`/`--project-level` are taken repo-wide. `setup-ide` passes `--global` explicitly (machine-level operation by nature).~~ **SUPERSEDED 20260909 (shipped 0.13.2): machine-level is the default and the flag is `--local`; `--global` is removed.** Shipped as designed in 0.13.0 and reversed after use — the commands are wanted machine-wide essentially always, and a project-local default cluttered every project with an identical copy of the same nine commands. The polarity-match with `cf config set --global` was the original rationale and did not hold up: config's machine-wide scope is the rare case worth spelling out explicitly, whereas for command install it is the norm. `setup-ide`/`init` now take the default rather than passing a scope. Do not re-derive the original polarity argument from this document.
