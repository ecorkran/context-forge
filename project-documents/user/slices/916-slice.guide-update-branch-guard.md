...
| otherwise: run `git merge-base --is-ancestor <trunk> HEAD` via `gitExec` | exit code 0 → `warn` (`ancestry: 'descends'`) — current branch contains trunk's tip in its history (typically a feature/test branch forked from trunk, or one that has since merged trunk back in; the guard does not distinguish these, since both are legitimate reasons to test an update before it lands on trunk). exit code non-zero → `warn` (`ancestry: 'unrelated'`) — no resolvable common ancestry; stronger warning text, but still confirmation-gated, not blocked, since forcing a hard stop here would remove a legitimate escape hatch for detached-HEAD or unusual repo states the user may need to work through manually. |

This applies identically whether or not `git.integration_branch` is set — when unset, `trunk = 'main'` and the same three rows evaluate the same way, just against `main`. No separate code path for the unset case.

`gitExec` already throws on non-zero exit for most commands (see `gitExec.ts:17-24`), which is the correct behavior for e.g. `git commit` failing. `git merge-base --is-ancestor` uses exit code as its actual return value (0 = true, 1 = false, >1 = error) — not a failure signal — so the guard must NOT call `gitExec` directly for this check; it needs a variant that captures exit code without throwing. Implementation note for task breakdown: add a small local wrapper in `branchGuard.ts` (using `execFile` directly, same no-shell-injection pattern as `gitExec`) rather than modifying `gitExec`'s throw-on-nonzero contract, since every other caller of `gitExec` relies on that contract.
...
- On `BranchGuardBlockedError`: print an error naming the configured trunk and current branch, with a suggested remediation (switch to the trunk branch, or unset `git.integration_branch` if that's not actually desired). Exit non-zero. No prompt — block is final.
...
- `evaluateBranchGuard()` has full unit test coverage of the decision table: trunk unset + on main, trunk unset + on unrelated branch, trunk unset + on descendant branch, trunk set + on trunk, trunk set + on main, trunk set + on descendant-of-trunk branch, trunk set + on unrelated branch.
